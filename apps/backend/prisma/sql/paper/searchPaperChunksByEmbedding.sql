-- Returns nearest paper chunks by embedding (cosine distance)
-- @param {Json} $1:query_embedding Embedding vector as JSON array (will be cast)
-- @param {Int} $2:limit Number of rows to return
-- NOTE: Requires pgvector extension. Uses <=> to match the HNSW index
-- (vector_cosine_ops). Casting JSON to vector assumes text '[]' style.
SELECT
  pc.id,
  pc."paperId",
  pc.idx,
