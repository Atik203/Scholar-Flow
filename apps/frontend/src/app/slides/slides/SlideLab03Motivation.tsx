...
const motivations = [
  {
    ...
    problem: "Researchers juggle 4+ disconnected applications daily, wasting valuable time switching between tools instead of doing research.",
  ...
  },
  {
    ...
    problem: "No centralized platform for team paper management leads to version conflicts, lost feedback, and siloed work across the team.",
  ...
  },
  {
    ...
    problem: "Researchers spend hours manually reading and analyzing papers when AI can extract key insights and findings in seconds.",
    ...
    <div className="w-full h-full bg-white p-8 flex flex-col">
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          ...
          <p className="text-xl text-black font-semibold">Why We Built ScholarFlow</p>
        </div>
      </div>
        ...
        <div className="col-span-2 grid grid-cols-2 gap-3">
          {motivations.map((item, index) => (
            <div key={index} className="bg-slate-50 rounded-xl p-4 border border-slate-200">
              ...
              <p className="text-xl font-semibold text-black mb-2 leading-snug">{item.problem}</p>
          ...
          <div className="space-y-2">
            {goals.map((goal, index) => (
              <div key={index} className="bg-white rounded-lg px-3 py-2.5 border border-slate-200 flex items-center gap-2">
                ...
                <span className="text-xl font-bold text-black">{goal.text}</span>
              </div>
            ))}
          </div>
        </div>
