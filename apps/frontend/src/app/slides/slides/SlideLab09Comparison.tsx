...
const features = [
  "Smart Paper Upload", "AI Summarization", "AI Chat with Papers", "Rich Text Editor",
...
function SupportIcon({ level }: { level: number }) {
  if (level === 2) return <Check className="w-6 h-6 text-emerald-600" strokeWidth={3} />;
  if (level === 1) return <Minus className="w-6 h-6 text-amber-600" strokeWidth={3} />;
    ...
    <div className="w-full h-full bg-white p-8 flex flex-col">
      ...
      <div className="flex-1 overflow-hidden">
        <table className="w-full border-collapse">
          <thead>
            <tr>
              <th className="text-left p-2 bg-slate-100 border-b-2 border-slate-300 font-bold text-black text-lg w-[180px]">Feature</th>
              {products.map((product, i) => (
                <th key={i} className="p-1 bg-slate-100 border-b-2 border-slate-300 text-center">
                  <span className={`inline-block px-2 py-1 rounded-lg text-sm font-bold ${product.color}`}>{product.name}</span>
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            {features.map((feature, fIndex) => (
              <tr key={fIndex} className={fIndex % 2 === 0 ? "bg-white" : "bg-slate-50/70"}>
                <td className="p-2 border-b border-slate-200 font-bold text-black text-base">{feature}</td>
                {products.map((_, pIndex) => (
                  <td key={pIndex} className={cn("p-1 border-b border-slate-200 text-center", pIndex === 0 && "bg-blue-50/60")}>
                    <div className="flex justify-center"><SupportIcon level={support[fIndex][pIndex]} /></div>
                  </td>
                ))}
              </tr>
            ))}
          ...
          <tfoot>
            <tr className="bg-gradient-to-r from-blue-100 to-indigo-100">
              <td className="p-2 font-bold text-black text-base">Features Supported</td>
              {products.map((_, pIndex) => {
                const fullSupport = support.filter((row) => row[pIndex] === 2).length;
                return (
                  <td key={pIndex} className="p-1 text-center">
                    <span className={`font-bold ${pIndex === 0 ? "text-blue-700 text-lg" : "text-black text-sm"}`}>{fullSupport}/12</span>
                  </td>
                );
              })}
            </tr>
