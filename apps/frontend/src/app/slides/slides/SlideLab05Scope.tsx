  ...
  return (
    <div className="w-full h-full bg-white p-8 flex flex-col">
      <div className="text-center mb-4">
        ...
        <p className="text-xl text-black font-semibold">What is Included and What is Not</p>
      </div>
          ...
          <div className="flex-1 space-y-3">
            {inScope.map((item, i) => (
              <div key={i} className="bg-white rounded-xl p-4 border border-emerald-100">
                ...
                <p className="text-xl font-semibold text-black leading-snug ml-10">{item.items}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="flex-1 space-y-3">
            {outScope.map((item, i) => (
              <div key={i} className="bg-white rounded-xl p-4 border border-red-100 flex items-start gap-3">
                ...
                <span className="text-xl font-semibold text-black leading-snug">{item}</span>
              </div>
            ))}
          </div>
        </div>
