...
export default function PaperEditorPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      router.push(`/dashboard/research/editor?paper=${result.data.paper.id}`);
                ...
                {papers.map((paper: any) => (
                  <motion.div key={paper.id} whileHover={{ y: -1 }} className="flex items-center justify-between border rounded-lg p-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <Link href={`/dashboard/research/editor?paper=${paper.id}`} className="font-medium text-sm hover:text-primary">{paper.title}</Link>
                    ...
                    <div className="flex gap-2">
                      <Button size="sm" variant="outline" asChild><Link href={`/dashboard/research/editor?paper=${paper.id}`}><Edit className="mr-1 h-3 w-3" />Edit</Link></Button>
                    </div>
                  </motion.div>
                ))}
              </div>
