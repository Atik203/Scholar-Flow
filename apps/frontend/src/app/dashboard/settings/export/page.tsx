...
export default function ExportPage() {
  ...
  const handleExport = async () => {
    ...
    try {
      // Real endpoint: GET /api/analytics/usage/export (backend, pro+)
      const res = await fetch(
        `${API_BASE_URL}/analytics/usage/export?format=${format}`,
        {
          method: "GET",
          headers: accessToken
      ...
      <div className="p-6 lg:p-8 max-w-2xl space-y-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Download your research usage report (papers, collections, activity).
          </p>
        </div>
