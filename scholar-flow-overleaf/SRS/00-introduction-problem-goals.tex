...
\begin{itemize}
    ...
    \item \textbf{Stakeholders \& Investors}: Demonstrates market validation through survey-backed insights (29 responses, 72.4\% need validation), competitive differentiation, and revenue model viability.
\end{itemize}

\subsection{Target Audience Demographics}

Based on comprehensive survey data from 29 academic researchers across 10 universities:
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|l|p{8cm}|}
        ...
        48.3\% Rarely Read         &
        Dual-persona design required: lightweight interface for casual users, power features (annotations, collections, AI summaries) for daily researchers (20.7\%). \\
        \hline
        \textbf{Institution}       &
        47.4\% UIU Concentration   &
...
Our survey of 29 academic researchers revealed critical pain points in current research workflows:
...
\begin{enumerate}
    \item \textbf{44.8\%} - Keeping notes and highlights organized across multiple papers
    \item \textbf{37.9\%} - Finding papers when needed (poor search/tagging in existing tools)
    \item \textbf{31.0\%} - Organizing papers into meaningful categories/collections
    \item \textbf{27.6\%} - Extracting key information efficiently (manual skimming wastes hours)
    \item \textbf{24.1\%} - Collaborating with team members (email attachments, version conflicts)
    \item \textbf{20.7\%} - Tracking reading progress across dozens of papers
\end{enumerate}

\textbf{Current Tool Fragmentation (Figure~\ref{fig:current-tools}):}

\begin{itemize}
    \item \textbf{34.5\%} use browser PDFs (no organization, lost tabs)
    \item \textbf{31.0\%} rely on desktop folders (``Desktop chaos'', no search)
    \item \textbf{31.0\%} store files in cloud drives (Drive/Dropbox/OneDrive --- files only, no metadata)
    \item \textbf{27.6\%} use no dedicated tool whatsoever
    \item \textbf{20.7\%} use reference managers (Zotero/Mendeley --- outdated UI, no AI)
    \item \textbf{13.8\%} use note-taking apps (Notion/Evernote --- not designed for papers)
\end{itemize}
...
\textbf{Validation}: 72.4\% of surveyed researchers indicated ``moderate to extreme need'' for a dedicated research paper management solution (Figure~\ref{fig:need-assessment}), while current tool satisfaction averages only 3.31/5 --- indicating significant room for improvement.
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|p{4cm}|p{6cm}|}
        ...
        Registration analytics, UIU campus penetration (47.4\% survey base)               \\
        \hline
        Feature Engagement                &
...
\begin{itemize}
    \item \textbf{Survey-Driven Requirements}: 29-response survey with 21 questions across demographics, feature priorities, and adoption intent
