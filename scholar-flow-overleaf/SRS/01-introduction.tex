...
Based on comprehensive survey data from 32 respondents (students and faculty) across multiple universities, ScholarFlow targets the following primary user segments:

\begin{itemize}[leftmargin=*]
    \item \textbf{Primary Segment (78.1\%)}: Undergraduate students conducting research projects, particularly in technology-centric fields
    \item \textbf{Secondary Segment (12.5\%)}: University faculty (lecturer/professor) supervising research and managing academic resources
    \item \textbf{Supporting Segments (\textasciitilde{}3.1\% each)}: Postgraduate students (Masters/MPhil), PhD researchers, and other academic roles
\end{itemize}

\textbf{Key User Characteristics:}
\begin{itemize}
    \item \textbf{Age Range}: 68.8\% between 22--25 years old (student and early-career researcher segment)
    \item \textbf{Academic Level}: 53.1\% are 3rd-year undergraduates at peak research intensity
    \item \textbf{Technical Proficiency}: Computer Science is the largest field (34.4\%), with additional tech-related fields represented
    \item \textbf{Geographic Focus}: Bangladesh market with strong UIU concentration (\textasciitilde{}60\% when combining duplicate UIU entries)
    \item \textbf{Current Pain Points}: Note-taking (46.9\%), discoverability (40.6\%), and organization (34.4\%) are the top challenges
\end{itemize}
...
\begin{enumerate}
    ...
    \item ScholarFlow Feasibility Survey Results (32 responses, November-December 2025)
...
\begin{description}[leftmargin=3cm, style=nextline]
    ...
    \item[\textbf{Chapter 2: System Study \& Information Gathering}]
          Documents the survey methodology, demographic analysis, current tool landscape, market validation, and pain points identified through comprehensive user research with 32 survey respondents.
